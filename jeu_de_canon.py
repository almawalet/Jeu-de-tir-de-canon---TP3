...
from configuration import ANGLE_MIN, ANGLE_MAX, PUISSANCE_MIN, PUISSANCE_MAX
from classement import Classement
from projectile import Projectile
from statut_jeu import StatutJeu
from exceptions import PuissanceInvalideException, AngleInvalideException, ErreurDeValeur
...
class JeuDeCanon:
    ...
    def tirer(self):
        ...
        if not (ANGLE_MIN <= self.angle <= ANGLE_MAX):
            raise AngleInvalideException()
        if not (PUISSANCE_MIN <= self.puissance <= PUISSANCE_MAX):
            raise PuissanceInvalideException()
        ...
        self.projectile.creer_projectile()
        self.projectile.initialiser_vitesses()
    ...
    def afficher_message_touche(self):
        ...
        self.message_touche = self.canvas.create_text(200, 150,
                                                      text="CIBLE TOUCHÉE!",
                                                      fill="green",
                                                      font=("Ariel", 20),
                                                      )
    ...
    def soumettre_score(self, nom, fenetre_parente):
        ...
        """
        if nom == "":
            messagebox.showinfo("Erreur", "Le nom ne peut pas etre vide.")
        else:
            self.classement.enregistrer_score(nom, self.score)
            fenetre_parente.withdraw()
